...
But for \KECCAK{}, there is also another kind of bruteforce attack possible. In the \KECCAK{} even if there is a collision in the capacity part of the hash state then also it is possible to yield a collision attack. Lets assume we have two messages $a, b$ such that they produce the following output and $f$ is our hash-function:
\[
...
\]
Both $M_1, M_2$ yield same state $S_1$ after applying hash function $f$, so a collision is found. But this depends on the collision in the $capacity$ part of the state which requires $2^{c/2}$ computations of the hash function by birthday attack. So the actual complexity for the collision attack of \KECCAK{} hash function with $d$-size hash and $c$ capacity is $min\left( 2^{c/2}, 2^{d/2}\right)$.
...
\subsection{Preimage Attack for $2$ Rounds of Round Reduced \KECCAK-$384$}
...
In my thesis, I have presented a preimage attack on the $2$ rounds of round-reduced \KECCAK-$384$. The attack is not practical but it is much better than the existing best-known attacks in term of the time complexity. The basic idea of the attack can be used to mount a practical preimage attack on a smaller variant i.e. $\Keccak[r:=400-192,\, c:=192]$ and $\Keccak[r:=800-384,\,c:=384]$. I have also presented a preimage attack on $3$ rounds of round-reduced \KECCAK-$256$, again this attack is also not practical. Further, in future, we will try to explore a practical attack for the $2$ or more rounds of round-reduced \KECCAK-$384$, \KECCAK-$512$.
