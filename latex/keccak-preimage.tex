...
\subsection{Discussion for symm hash attack}
on 3 round Keccak-384 one can imagine a preimage attack for "weak" hash values as follows: 

Assume the hash can be generated by a close to symmetric state. To be specific, we can use the 2.5-round Characteristic 3, adapted to i=32. It has probability 2^{-24}.

If after computing the first 5 lanes of the hash by \chi layer backwards, they follow the internal difference of (adapted) Characteristic 3, we assume it can be generated by a message with initial internal difference INT as in this characteristic.

Guess there are about 2^192 hash values with this property. 

Now we try about 2^192 messages with internal difference INT, but which satisfy additional conditions so that the differential trail follows Characteristic 3 with probability (close to) 1.

We assume that the map from messages to hashes is (almost) injective. Thus about 2^192 different  hashes will be generated that fit the right pattern in 5 lanes. One of them is likely to be the correct hash value.

If correct, the complexity would be about 2^192 evaluations of 3-round Keccak-384.

Probably, we should also take 6-th lane into account somehow for better filtering of potentially weak hashes, 

Also, we might enlarge the fraction of weak hash values by building a table with possible internal differences after 2.5 rounds together with the corresponding conditions. This table would have about 2^24 entries?





> Problem: Does my sketched attack on 3-round Keccak-384 work, and if so, can the complexity be reduced somehow?

I have some doubts regarding the attack described by you :
1. If the assumption that the map from message to hash is injective, doesnt hold true much then there is possibility that a preimage can't be found. 

2. The final state of the characteristic is not a complete symmetric hash, how do we deal with this? As the characteristic would yield a hash similar to the final state in the trail and we are working on symmetric hashes. How will this gap be filled?

3. Even if the method is lucky enough to fit the pattern in the 5 lanes, how does it satisfy 6th lane?
I know there are 2^{192} distinct symmetric hashes. 

> Also, we might enlarge the fraction of weak hash values by building a table with possible internal differences after 2.5 rounds together with the corresponding conditions. This table would have about 2^24 entries?
Yes, the table should have 2^{24} entries.

If it suits, characteristic 4 can also be used for 3 round attack (it is defined for i=32)


> what can be done for 4 rounds when using (close to) symmetric messages?
A one good one round extension to a 3 round characteristic first would be a suitable step? Characteristic-4 seems a good candidate for extension. But how are extensions done is yet to be seen (acc. to me)

>Are there more weak hash values for 2-round Keccak-384 than the 2^192 values?
In my view for i=32. Every lane can choose any of 2^{32} value and then the same is repeated in the other part of the lane.
So there are 2^{32*6} such hashes. 

> we shall need  detailed investigations, maybe based on experiments or analytical considerations.
Can you suggest any experiment which I can start to look into? I will try to do detailed analysis.
\end{document}
