...
\begin{enumerate}
    ...
    \item For both of them input to the step $\chi$ of the third round are all linear, hence the method for matching a hash bits by linearizing the terms corresponding to these hash bitscan work for this too. As shown in section 6.3.
    
    \item In this we linearize the output of $\theta$ of 4th round by guessing bits input to $\theta$. We need 10 guesses to match 1-bit, but 
    ...
    \item Then we have 127 and 64 variables resp, we can match $2*\frac{t-5}{8}$ bits of a hash value if we have t variables.
    \item For Keecak-224 : matched bits $= 30$
