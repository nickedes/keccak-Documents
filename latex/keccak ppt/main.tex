...
\begin{frame}
	...
	\begin{itemize}
		...
		\pause
		\item In year 2005, First practical collision attacks on
		\begin{itemize}
...
%		\item in the same year, a practical collision attack on \SHA-$0$ and \SHA-$1$ was published by Xiaoyun Wang \etal
		\pause
		\item NIST was worried about the security of hash functions
		
		\item Though by that time \SHA-$2$ family of hash functions was standardized
		\pause
		\item But as \SHA-$2$ was also based on Merkle-Damgard construction like \SHA-$0$, \SHA-$1$
...
\begin{frame}
	\frametitle{State}
	\begin{itemize}
		\item The state input to \KECCAK-$f\left[b\right]$ consists of $b$ bits, the state is divided into slices
...
\begin{frame}
	\frametitle{Round Function}
	\begin{itemize}
		\item The round function $p$ in \Keccak{} comprises of $5$ steps
		\item In each of which the state undergoes transformations specified by the step mapping
...
\begin{frame}
	...
	\begin{itemize}
		\item XORs each bit in the state with the parities of two neighboring columns
		...
		\end{align}
		\item $P\left[x,\; z\right]$ represents the parity of the column $(x,\; z)$
		\[
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item $B\left[x, \,y,\, z\right] = A\left[x, \,y, \,z + \rho\left(x, y\right) \bmod w\; \right]$
		\item $\rho\left(x,\; y\right)$ is the constant for lane $\left(x,\; y\right)$
		\item The constant value $\rho\left(x,\; y\right)$ is specified for each lane in the construction of \Keccak{}
		\item $\rho$ is also a linear step mapping 
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\item $\pi$ ({\bf pi}): It permutes the positions of lanes
...
\begin{frame}
	...
	\begin{itemize}
		\item Round in \Keccak{} is given by ${\tt Round}\left(A,\; i_r\right) = \iota\left( \chi\left( \pi \left( \rho \left( \theta \left( A \right) \right) \right) \right),\; i_r\right)$
		\item Consists of $n_r$ number of rounds
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item The value of $b = 1600$, so we have $l = 6$
		...
		\item Where $r=1600-c$ and the capacity $c$ is chosen to be twice the size of hash output $d$
		\item To avoid generic attacks with expected cost below $2^d$
		\item The hash function with output length $d$ is denoted by 
		\begin{eqnarray}
		\mbox{\Keccak-$d$}  &=& \Keccak\left[r:=1600-2\cdot d,\;c:=2\cdot d\right]
		\end{eqnarray}
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Rule is that the input string is appended with a $1$ bit followed by some number of $0$ bits and followed by $1$ bit
...
\begin{itemize}
	...
	\item $a_i$ are the output bits of $\chi$ and $a_i^\prime$ are the input bits
	
	\item Guo \etal observed that when $4$ out of $5$ output bits of $\chi$ are known
	\item Then we can obtain $4$ linear relations in terms of $a_i'$
	...
	\item If the values of $a_0,\;a_1,\;a_2,\;a_3$ are known
	\item Using the Equation~\ref{eq:a0}, we can eliminate the expression $a_4$ from the rest of the equations 
	\item Hence obtain $4$ linear relations
...
\begin{frame}
	...
	\begin{itemize}
		\item State is represented by $25$ lanes
	...
	\begin{itemize}
		...
		\item $2$ rounds of \KECCAK{}$\left[r:=800-384,\;c:=384\right]$\\
			\begin{itemize}
				...
				\item Based on the idea of Linear structures given by Jian Guo \etal in $2016$
			\end{itemize} 
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Aim : Control the diffusion
		\item By adding constraints on message bits
		\item Conditions to make column parity zero:
		\begin{align}\nonumber
...
\item Fix $d_0,d_1$ as constants (with condition that $d_0=d_1$).
\pause
\item In state $2$, we have $11\cdot 32$ free variables.
\pause
\item And we need to satisfy $7\cdot 32$ Boolean equations and $4\cdot 32$ column parity conditions.
\pause
\item So, we expect a solution.
\pause
\item We do find the possible solution subspace. 
\end{itemize}
...
\pause
\item We first find the set of input message bits which satisfy the small collection of consecutive slices of state $3$.
\pause
\item  We then merge the solutions to find message bits which satisfy large collection of consecutive slices.
\end{itemize}
...
\pause
\item Thus the total possible solutions for this $3$-slice  $=2^{33-2\cdot 7}=2^{19}$
\end{itemize}
\end{frame}
...
\pause
\item Consider, for example, the first two $3$-slices (first $6$ slices). It  contains the following message bits:
\begin{itemize}
...
\pause
\item Since, we already have the correct bit values of slice~$3$ of the State~$3$, and there is dependency between the above message bit variables, we end up having total possible solutions = $2^{2 \cdot 19 - 2 - 7}=2^{29}$.
...
\pause
\item Consider, for example, the first $12$ slices. It contain the following message bits:
\begin{itemize}
...
\pause
\item Similar to $6$-slice, the bit variables are not independent

\item The bit variables $a_0,a_1,a_2[6,7,8,9], b_0,b_1,b_2[12]$, $e_0[27-31]$ and $e_1[27-31]$ are dependent
\pause
\item Hence, the total possible solutions = $2^{2 \cdot 29-10-7}=2^{41}$
\end{itemize}
\end{frame}
%-----------------------------------------------------------------
...
\item The total number of possible solutions is equal to  $2^{2 \cdot 41-34-7}=2^{41}$
\end{itemize}
\end{frame}
...
\begin{itemize}
\setlength\itemsep{2em}
\item Thus, using above method, we find the possible solutions for the remaining $6$ slices and $2$ slices
\pause
\item For finding the solution for the last consicutive $8$ slices, we merge possible solution of its consituent $6$-slice and $2$-slice
\pause
...
\pause
\item A practical attack with attack complexity of $2^{44}$
\pause
