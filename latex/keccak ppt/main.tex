...
\begin{frame}
	...
	\begin{itemize}
		\item A hash function is of the form $H:\{0,1\}^* \rightarrow \{0,1\}^n $.
		\pause
		\item It is a deterministic	function.
		\item It takes as input an arbitrary size string and outputs a fixed size ($n$) string.
		\pause
		\item It is used in cryptographic applications such as Authentication, Digital Signatures and Integrity etc..
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\pause
		\item If we have $A$ as the input state to $\rho$ then the output state $B$ is:
		\begin{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item $\pi$ is a linear step mapping.
	\end{itemize}
\end{frame}
